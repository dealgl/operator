...
@Component
@Result(type = "json")
public class GetResumeInfoAction {

	final static Log log = LogFactory.getLog(GetResumeInfoAction.class);
   ...
   @Action("get-resume-info")
   public String execute(){
	   	 info =OracleDBManager.getInstance().getResumeInfoById(face_id); 
	   	 addInfo =OracleDBManager.getInstance().getResumeAddInfoById(face_id);
	   	 countIndex = index;
  ...
  /* 
   public String invokeAutoCall(){
	    PhoneCallService callService = new PhoneCallService();
	    PhoneCallInterface port = callService.getPort(PhoneCallInterface.class);
		
		CallRequest callRequest = new CallRequest();
		callRequest.setFrom("923950");
		callRequest.setTo("089271466565");
		callRequest.setTimeout(10);
		//callRequest.setTo(phone.size() < 8 ? phone : "08" + phone);

		if(port.makeCall(callRequest)){
			this.message = "Звонок...";
			success = true;
			}
		else{
