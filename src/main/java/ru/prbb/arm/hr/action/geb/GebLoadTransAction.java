...
@Component
@Result(type = "json")
public class GebLoadTransAction {

	final static Log log = LogFactory.getLog(GebLoadTransAction.class);
    ...
    public String execute(){
    	...
    	transferFiles = OracleDBManager.getInstance().getTransLoadTmpList(String.valueOf((RequestContextHolder.currentRequestAttributes().getAttribute("v_user_id", RequestAttributes.SCOPE_SESSION))));
    	...
    	for (int i=0;i<transferFiles.size();i++){
    		...
    		)
    		{
    			...
    			if (crAccount.equals("ОВП") || (crAccount=="ОВП")){
    				cnt_cells=0;
    				cnt_all=0;	
    			...
    			if (dbAccount.equals("ОВП") || (dbAccount=="ОВП")){
    				cnt_cells=0;
    				cnt_all=0;
    				for (int k=0;k<transferFiles.size();k++){
    					crAccount1=transferFiles.get(k).get("cr");
