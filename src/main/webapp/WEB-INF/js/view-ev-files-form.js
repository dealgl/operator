...
(function() {
	...
	var fileGrid = new Ext.grid.GridPanel( {
		...
		tbar : [
					...
					handler : function(self) {
						...
						if (idn == '') {
							Ext.MessageBox.show( {
								...
								Ext.each(sm.getSelections(), function(item) {
									window.open(String.format('ev/view-file-ev-proceed.html?type={0}',item.data.files));
								});
				...
				
				{
					...
					handler : function(self) {
						...
						if (idn == '') {
							...
							window.open(String.format('ev/view-file-ev-proceed.html?type={0}',idn));
						}
					}
				},
				...
				if (ids == '') {
					...
					Ext.Ajax.request( {
						url : 'ev/correct-transfer-pay-ev-main.html',
						params : {
							path : ids
						},
						timeout : 1000000000,
	...
	container = new Ext.FormPanel( {
		id : 'view-ev-files-form-component',
		frame : true,
		closable : true,
		header : false,
		autoScroll : true,
