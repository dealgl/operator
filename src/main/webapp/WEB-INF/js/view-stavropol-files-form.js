...
(function() {
			...
			buttons : [ {
				...
				handler : function(self) {
					Ext.getCmp(_winFiles).getForm().submit( {
						url : 'stavropol/save-stavropol-file.html',
	...
	var fileGrid = new Ext.grid.GridPanel( {
		...
		tbar : [
...

				{
					...
					handler : function(self) {
						...
						if (idn == '') {
							...
							window.open(String.format('stavropol/view-file-stavropol-proceed.html?id={0}&type={1}',ids,idn));
						}
					}
				},
				...
				if (ids == '') {
					...
					Ext.Ajax.request( {
						url : 'stavropol/correct-transfer-pay-stavropol-main.html',
						params : {
							path : ids
						},
						timeout : 1000000000,
	...
	container = new Ext.FormPanel( {
		id : 'view-stavropol-files-form-component',
		frame : true,
		closable : true,
		header : false,
		autoScroll : true,
		...
		loadData : function(data) {
			this.setTitle("Экспресс-Волга (Ставропольский филиал)<br/><b>" + "Документы" + "</b>");
			this.data = data;
			/*if (data['file'][0]) {
				store.loadData(data);
			}*/
