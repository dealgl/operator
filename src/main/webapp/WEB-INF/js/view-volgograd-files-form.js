...
(function() {
			...
			buttons : [ {
				...
				handler : function(self) {
					Ext.getCmp(_winFiles).getForm().submit( {
						url : 'volgograd/save-volgograd-file.html',
	...
	var fileGrid = new Ext.grid.GridPanel( {
		...
		tbar : [
				...
				"-",
				{
					...
					handler : function(self) {
						...
						if (idn == '') {
							...
							window.open(String.format('volgograd/view-file-volgograd-proceed.html?id={0}&type={1}',ids,idn));
						}
					}
				},
				"-",
				...
				if (ids == '') {
					...
					Ext.Ajax.request( {
						url : 'volgograd/correct-transfer-pay-volgograd-main.html',
						params : {
							path : ids
						},
						timeout : 1000000000,
	...
	container = new Ext.FormPanel( {
		id : 'view-volgograd-files-form-component',
		frame : true,
		closable : true,
		header : false,
		autoScroll : true,
		...
		loadData : function(data) {
			this.setTitle("Экспресс-Волга (Волгоградский филиал)<br/><b>"
					+ "Документы" + "</b>");
			this.data = data;
			/*if (data['file'][0]) {
				store.loadData(data);
