...
(function() {
						...
						items : [
								...
								},
								{
									...
									handler : function(self) {
										menu.showAbstractOfCardSKS(data['contr_id'],'A',Ext.getCmp(_dateS).getRawValue(),Ext.getCmp(_dateE).getRawValue());  
										...
										/*filterForm
												...
												.submit(
														{
															url : 'webservices/card-transaction-list-main.html',
															waitMsg : 'Выборка данных...',
															timeout : 60000,
															 params : {contr_id : data['contr_id'],p_contr_type : 'A'},
															success : function(
																	form,
																	action) {
																grid
								...
								
								{
									...
									handler : function(self) {
										window.open(String.format("http://192.168.68.104:6666/xmlpserver/UPK/PUBLIC/COLLECTION/CardTransactionListPrint/CardTransactionListPrint.xdo?id=tech&passwd=tech1&_xmode=4&xdo_BI_XF=rtf&_xf=rtf&_xpt=1&p_user_id="+userId+"&p_date_from="+Ext.getCmp(_dateS).getRawValue()+"&p_date_to="+Ext.getCmp(_dateE).getRawValue()+"&p_contr_type=A&p_contr_id="+data['contr_id'])); 
	...
	return new Ext.Panel( {
    	...
    	this.setTitle("Выписка по счету");        	
    	this.data = data;
        	store.loadData(data);
